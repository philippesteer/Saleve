function h = hillshadeTri(x,y,z,DT)
% COmpute hillshade on an irregular grid (triangulated)
...
% lighting azimuth
az = 360.0-az+90; %convert to mathematic unit 
az(az>=360)=az-360;
az = az * (pi/180); %  convert to radians
%lighting altitude
el = (90-el) * (pi/180); % convert to zenith angle in radians
%% calc slope and aspect (radians)
[fx,fy] = trigradient(x,y,z,DT.ConnectivityList);
[asp,grad]=cart2pol(fy,fx); % convert to polar coordinates
grad=atan(zf*grad); %steepest slope
asp(asp<pi)=asp(asp<pi)+(pi/2);
asp(asp<0)=asp(asp<0)+(2*pi);
%% hillshade calculation
h = 255.0*( (cos(el).*cos(grad) ) + ( sin(el).*sin(grad).*cos(az-asp)) ); % ESRIs algorithm
